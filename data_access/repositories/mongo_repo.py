...
class MongoRepository:
    ...
    def obtener_agente_por_nombre(self, nombre):
        ...
        return self.agents.find_one({"nombre": {"$regex": f"^{nombre}$", "$options": "i"}})
    ...
    def obtener_tickets_por_estado_y_entidad(self, entidad_campo, entidad_valor, estado):
        """
        Ejemplo: Traer tickets 'Cerrados' del Agente 'Juan'
        entidad_campo: 'agente_id' o 'empresa_id'
        """
        query = {
            entidad_campo: entidad_valor,
            "estado": estado
        }
