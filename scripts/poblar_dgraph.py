...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../src/presentation')))
...
def poblar_dgraph():
    ...
    base_csv = os.path.abspath(os.path.join(os.path.dirname(__file__), '../data/csv/dgraph'))
    all_uids = {}
    ...
    def cargar_entidades(nombre_csv, id_key, type_name):
        ...
        data = []
        try:
            data = [row for row in csv.DictReader(open(f"{base_csv}/{nombre_csv}", 'r', encoding='utf-8'))]
        ...
        for row in data:
            ...
            mutation = {'uid': f"_:{row[id_key]}", id_key: row[id_key], 'dgraph.type': type_name}
    ...
    try:
        relaciones_data = [row for row in csv.DictReader(open(f"{base_csv}/relaciones.csv", 'r', encoding='utf-8'))]
    except FileNotFoundError:
    ...
    for row in relaciones_data:
        ...
        if origen_uid and destino_uid:
            ...
            relaciones_nquads.append(relacion)
             ...
             print(f"   🤷‍♀️ Relación omitida (UIDs no encontrados): {origen_id} -> {destino_id}")
