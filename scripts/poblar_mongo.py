...
def leer_csv_mongo(filename):
    ...
    base_csv = os.path.abspath(os.path.join(os.path.dirname(__file__), '../data/csv/mongo'))
...
def poblar_mongo():
    ...
    for row in leer_csv_mongo('empresas.csv'):
        datos = {
            "id": row['idEmpresa'], "nombre": row['nombre'], "correo": row['correo'], 
    ...
    for row in leer_csv_mongo('agentes.csv'):
        ...
        fecha_ingreso = datetime.strptime(row['fechaIngreso'].replace("Z", ""), '%Y-%m-%dT%H:%M:%S')
        
        datos = {
            "id": row['idAgente'], "nombre": row['nombre'], "email": row['correo'], 
            "rol": row['rol'], "activo": True, "fecha_creacion": fecha_ingreso
        }
    ...
    for row in leer_csv_mongo('clientes.csv'):
        datos = {
            "id": row['idCliente'], "nombre": row['nombre'], "email": row['correo'], 
            "telefono": row['telefono'], "fecha_registro": datetime.now()
        }
    ...
    for row in leer_csv_mongo('tickets.csv'):
        ...
        fecha_cierre = datetime.strptime(row['fechaCierre'], '%Y-%m-%dT%H:%M:%S') if row.get('fechaCierre') else None
        
        datos = {
            "id": row['idTicket'], "titulo": row.get('titulo', 'Ticket Importado'), 
            "descripcion": row.get('descripcion', 'N/A'), "prioridad": int(row['prioridad']), 
            "cliente_id": row['idCliente'], "agente_id": row.get('idAgente'),
            "estado": row['estado'], "fecha_creacion": datetime.strptime(row['fechaCreacion'], '%Y-%m-%dT%H:%M:%S'),
