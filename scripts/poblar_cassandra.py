...
def _leer_csv_cassandra(filename):
    ...
    base_csv = os.path.abspath(os.path.join(os.path.dirname(__file__), '../data/csv/cassandra'))
        ...
        try:
            ...
            for col in columns:
                ...
                # Conversión de Fecha a TIMEUUID
                    ...
                    fecha_datetime = datetime.fromisoformat(value.strip().replace("Z", "")) 
                    value = uuid_from_time(fecha_datetime)
                # Conversión a INT si es un campo numérico
                elif col in ['estado', 'prioridad', 'estadoEnEmpresa', 'estadoCuenta'] and value:
                    value = int(value)
...
def poblar_cassandra():
    ...
    _poblar_tabla_cassandra(session, 'empresa', data_empresa, ['idEmpresa', 'fecha', 'idTicket'], ['idEmpresa', 'idTicket'])

    # 2. AGENTE
    data_agente = _leer_csv_cassandra('agentes.csv')
    _poblar_tabla_cassandra(session, 'agente', data_agente, ['idAgente', 'fecha', 'estadoEnEmpresa'], ['idAgente'])
    
    # 3. CLIENTE
    data_cliente = _leer_csv_cassandra('clientes.csv')
    _poblar_tabla_cassandra(session, 'cliente', data_cliente, ['idCliente', 'fecha', 'estadoCuenta'], ['idCliente'])

    # 4. HISTORIAL TICKETS (Comentarios, Agente, Estado, Prioridad)
    data_com = _leer_csv_cassandra('tickets_com.csv')
    _poblar_tabla_cassandra(session, 'ticket_com', data_com, ['idTicket', 'fecha', 'idAgente', 'comentario'], ['idTicket', 'idAgente'])

    data_age = _leer_csv_cassandra('tickets_age.csv')
    _poblar_tabla_cassandra(session, 'ticket_age', data_age, ['idTicket', 'fecha', 'idAgente'], ['idTicket', 'idAgente'])

    data_est = _leer_csv_cassandra('tickets_est.csv')
    _poblar_tabla_cassandra(session, 'ticket_est', data_est, ['idTicket', 'fecha', 'estado'], ['idTicket'])

    data_prio = _leer_csv_cassandra('tickets_prio.csv')
    _poblar_tabla_cassandra(session, 'ticket_prio', data_prio, ['idTicket', 'fecha', 'prioridad'], ['idTicket'])
